...
####❀确保系统硬件能通过Cinebench，[Intel Burn Test](https://intelburntest.en.lo4d.com/windows)，[Furmark](https://geeks3d.com/furmark)等烤机测试，最好整点大型游戏（测试USB总线与CPU同时负载）以确保系统几乎不可能死机，然后再考虑内存超频
 - 如果还没买内存（或主板），建议先看 [上期教程-内存购买前的超频教程](https://nazorip.site/archives/925)

-----

!!!
<font size=4><b>工作频率</b></font>
!!!
...
!!!
<font size=4><b>内存结构</b></font>
!!!
**分库Bank：** 由对应的一套操作列和缓存行组成的小分区，位于内存芯片DRAM内，DDR3的DRAM中有8个分库，DDR4中有16个分库，每四个一组，一组分库叫Bank Group
 - DDR5有32个分库
 - Bank会被映射到系统层，因此也有着"银行"的意思

**分阶Rank：** 一簇DRAM芯片，常见的2Rx4、2Rx8中的R，一般PCB一侧为一个Rank
 - 同DIMM里，DRAM越多，Rank则越多，处理器能用的Bank就越多，性能越高，造价越高
 - 4RxN的DIMM存在但少见，可能在HPC服务器中能找到
![Memory Rank Numbers](https://img4.yesya.net/img/5/2023-06/1R-2R-4RankMem.jpg)

**Prechange初始化：** 将缓存行"关闭"，回到待机Idle模式
**Activate启用：** 将缓存行从待机Idle状态中"打开"，完成后的缓存行可以读写
**Refresh刷新：** 给整个Rank中所有的DRAM"充电"，保证数据不受时间变化而损坏

**缓存行Column/纸线Wordline：** 读写数据的位置，位于内存芯片DRAM内，分库中的操作列下
**操作列Row/笔线Bitline：** 简单地说，操作信号进入对应缓存行前要流经的"树干"，位于内存芯片DRAM内
Wordline和Bitline是微观/电路上构成"缓存行"和"操作列"的结构
 - 每条Wordline横向连接着分库中的内存单元行（中的晶体管开关），负责启用/关闭Bitline连接到内存单元（中代表0和1电容）。如果失效，则Bitline无法读写内存单元的状态，或者会写入/读取进错行的内存单元中（的存储电容） 
   - 晶体管=Transister，电容=Capacitor，所以这种内存单元叫1T1C Cell
   - 目前的DRAM好像已经淘汰1T1C Cell结构了，但方便举例子
![MemoryCell-WL-BL.jpg](https://img4.yesya.net/img/5/2023-06/MemoryCell-WL-BL.jpg)
...
![ASUS](https://img4.yesya.net/img/5/2023-06/DDR5HynixOC%20-%20NoFastBoot%201.png)<br>![ASUS笔记本](https://img4.yesya.net/img/5/2023-06/DDR5HynixOC%20-%20NoFastBoot%202.png)

-----

!!!
<font size=4><b>电压</b></font>
!!!
**漏极Drain：** 此处指主板金属氧化物场效应晶体管MOSFET中，源极Source，漏极，栅极Gate三（或4）针之一，栅极=高频开关，源极=电源（电脑上是12V），漏极=取电（输出1.xV）
 - **漏极电压Drain Voltage学名就是VDD**
...
![EKWB-PDUVRM.jpg](https://img4.yesya.net/img/5/2023-06/EKWB-PDUVRM.jpg)
...
**集电极Collector：** 在电脑上和"VDD"电压完全相同，在电气学上是把MOSFET换成了双极性晶体管BJT，名字换成射端Emitter，基极（Base）和集电极（Collector）。BJT功耗更高，但比MOSFET更适合放大电流，所以常见于音响功放中
 - **集电电压Collector Voltage学名就是VCC**
 - ~~因奈特莉~~Intel做处理器的年头久了，所以继续叫VCC可能只是沿用非常老的电气学名而已
...
!!!
<font size=4><b>时序（附海力士/Hynix DRAM预设）</b></font>
!!!
...
**MSI主板设置图例：**
![MSI mobo timing exp](https://img4.yesya.net/img/5/2023-06/MSI_GSkill_F5-6400J3239G16G_Subtimi.png)

-----

!!!
<font size=4><b>DDR5 ODT阻抗设置（仅小道消息）</b></font>
!!!
**末端电阻ODT：** 见[上期教程-内存购买前的超频教程](https://nazorip.site/archives/925)
...
!!!
<font size=4><b>时钟信号发生器ClkDrv设置（仅小道消息）</b></font>
!!!
...
!!!
<font size=4><b>超频前后对比</b></font>
!!!
 - 使用了锐龙7700X，B650平台，F5-6400J3239G16G内存（6200MT/s 2x16GB 32-29-29-102 CR1T），AIDA64的内存跑分功能
 - **超频前：**
![BeforeOC.png](https://yesya.oss-ap-northeast-2.aliyuncs.com/5/2023-06/BeforeOC.png)
 - **超频后：**
![AfterOC.png](https://yesya.oss-ap-northeast-2.aliyuncs.com/5/2023-06/AfterOC.png)

-----

**关于DDR5的ECC校验：**
 - DDR5内存比DDR4桌面端强在使用了On-die ECC校验，但和真的ECC芯片原理/Off-die ECC不同，是多装了颗内存芯片来实现的，所以内存超频后的校验效果存疑
...
<style>
    ...
    body {font-family: "Roboto","Helvetica","Arial","sans-serif"; font-size: 100%; background-image: url('https://yesya.oss-ap-northeast-2.aliyuncs.com/5/2023-06/30_Background.jpg'); background-repeat: no-repeat; background-position: top left; background-attachment: fixed; background-size:cover}
</style>
!!!
