...
<p align="center"><b>DIMM（内存条） > Rank（分阶/分簇） > DRAM（内存芯片） > Bank（分库） > Cell（内存单元）</b></p>
...
**Prechange初始化：** 将缓存行"关闭"，回到待机Idle模式 <br>
**Activate启用：** 将缓存行从待机Idle状态中"打开"，完成后的缓存行可以读写 <br>
**Refresh刷新：** 给整个Rank中所有的DRAM"充电"，保证数据不受时间变化而损坏 <br>
...
**末端电阻ODT：** 见[上期教程-内存购买前的超频教程](https://nazorip.site/archives/925) <br>
